...
enum CardTransitionType {
    ...
    var cardMode: CardViewMode { return self == .presentation ? .card_CardViewMode : .full_CardViewMode }
...
class CardTransitionManager: NSObject {
    ...
    private func createCardViewCopy(cardView: CardView) -> CardView {
        let cardModel = cardView.eventData
        cardModel.viewMode = transition.cardMode
        let cardViewCopy = CardView(eventData: cardModel)
        return cardViewCopy
    }
...
extension CardTransitionManager: UIViewControllerAnimatedTransitioning {
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        guard let cardView = (transition == .presentation) ? (fromView as! EventViewController).selectedCellCardView() :
                                                             (toView as! EventViewController).selectedCellCardView()
                            else { return }
        ...
        NSLayoutConstraint.activate([
            closeButton.topAnchor.constraint(equalTo: cardViewCopy.containerView.topAnchor, constant: 20.0),
            ...
            closeButton.rightAnchor.constraint(equalTo: cardViewCopy.containerView.rightAnchor, constant: -20.0)
        ])
    ...
    func moveAndConvertToCardView(cardView: CardView, containerView: UIView, yOriginToMoveTo: CGFloat, completion: @escaping () -> ()) {
        ...
        if transition == .presentation {
            shrinkAnimator.addCompletion { _ in
                ...
                cardView.updateLayout(viewMode: self.transition.next.cardMode)
